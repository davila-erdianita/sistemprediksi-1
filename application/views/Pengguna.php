...
<div class="col-md-10 p-5 pt-2 ml-auto" style="background-color: white;">
	<div class="card">
	  <div class="card-body">		
	    <h3><i class="fa-solid fa-gauge mr-2"></i> DATA PENGGUNA</h3><hr>
	    <div class="row pt-2">
                <?php if($this->session->flashdata('status_pengguna')){?>
                            <div class="alert alert-success">
                                <?= $this->session->flashdata('status_pengguna')?>
                            </div>
                        <?php }elseif($this->session->flashdata('gagal_pengguna')){?>
                          <div class="alert alert-danger">
                                <?= $this->session->flashdata('gagal_pengguna')?>
                            </div>
                        <?php }?> 
    	</div>
	    <div class="row">
	    <div class="col-md-7 mt-2">
	    	<a href="<?php echo base_url();?>Pengguna/FormPengguna" class="btn btn-outline-primary btn-sm mb-3">Tambah Data</a>
		</div>
		<!-- <div class="col pl-5">
			...
			<div class="col">
				<div class="table-responsive">
					  <table class="table" id="datatables">
						  ...
						  <tbody><?php $i=1; foreach ($all_user as $data) : ?>
						    <tr>
						      <td><?= $i++;?></td>
						      <td><?= $data['username'];?></td>
						      <td><?= $data['role'];?></td>
						      <td><?= $data['status'];?></td>
						      <td><a href="<?php echo base_url()."Pengguna/delete_data/".$data['id_user'];?>" id="del" class="btn btn-sm btn-outline-danger"><i class="fas fa-trash-alt"></i></a>
                                  <a href="<?php echo base_url()."Pengguna/UpdateForm/".$data['id_user'];?>" id="edit" class="btn btn-sm btn-outline-info"><i class="fas fa-edit"></i></a>
                              </td>
						    </tr>
						<?php endforeach; ?>
						  </tbody>
